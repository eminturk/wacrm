    ...
    async function fetchData() {
      try {
        const [broadcastRes, recipientsRes] = await Promise.all([
          fetch(`/api/broadcasts/${broadcastId}`, { cache: 'no-store' }),
          fetch(`/api/broadcasts/${broadcastId}/recipients`, {
            cache: 'no-store',
          }),
        ]);
        const [broadcastData, recipientsData] = await Promise.all([
          broadcastRes.json(),
          recipientsRes.json(),
        ]);

        if (!broadcastRes.ok) {
          throw new Error(broadcastData.error || 'Failed to load broadcast');
        }
        if (!recipientsRes.ok) {
